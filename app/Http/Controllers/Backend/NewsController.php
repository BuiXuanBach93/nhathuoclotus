...
class NewsController extends AppBaseController
{
    ...
    public function edit($id)
    {
        ...
        $currentUser = Auth::user();
        if ($currentUser->role != User::ROLE_ADMIN && $currentUser->role != User::ROLE_ADMIN_EDITOR && $currentUser->id != $news->user_id && $currentUser->id != $news->assignee_id) {
    ...
    public function update($id, UpdateNewsRequest $request)
    {
        ...
        // Clean redis key
        $cachedKeyMobile = "ntlt_mobile_".$news->slug;
        $cachedKeyDesktop = "ntlt_desktop_".$news->slug;
        try {
    ...
    public function updateCanonical()
    {
        ...
        Redis::set('ntlt_canonical_urls', json_encode($canonicals));
     ...
     public function anyDatatables() {
        $currentUser = Auth::user();
        if ($currentUser->role == User::ROLE_ADMIN || $currentUser->role == User::ROLE_ADMIN_EDITOR) {
            ...
            ->select(
                ...
                'news_categories.slug as category_slug',
                'users.name as assignee_name'
            )->where('news.user_id','=',$currentUserId)->orWhere('news.assignee_id','=',$currentUserId);
        }
