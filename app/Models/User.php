...
class User extends Model
{
    ...
    // Nhân viên tư vấn
    protected static $adminEditor = 5;

    const ROLE_ADMIN = 1;
    const ROLE_WRITER = 2;
    const ROLE_STOCKER = 3;
    const ROLE_ADVISOR = 4;
    const ROLE_ADMIN_EDITOR = 5;
    ...
    public static function isAdvisor($role) {
     ...
     public static function isAdminEditor($role) {
        if ($role == User::$adminEditor) {
            return true;
        }
    ...
    public function getEditors() {
        $editors =  User::where('role', User::ROLE_WRITER)->orWhere('role', '=', 5)->get();
        $result = [];
